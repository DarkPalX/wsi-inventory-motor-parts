...
class FileHelper
{
    ...
    public function move_to_files_folder($file, $folderPath)
    {
    ...
    private function make_unique_file_name($folder, $fileName)
    {
        ...
        $extension = array_pop($fileParts); // Get the file extension
        $name = implode('.', $fileParts);   // Get the base name without extension
        
        $count = 2; // Start with a count of 2
        $newFilename = $name.' ('.$count.').'.$extension; // Initial new filename
        
        // Check if the file already exists in the specified folder
        while (Storage::disk('public')->exists($folder.'/'.$newFilename)) {
            $count += 1; // Increment the count
            $newFilename = $name.' ('.$count.').'.$extension; // Generate a new filename with the updated count
        }

        return $newFilename; // Return the unique filename
    }
