...
class ListingHelper
{
    ...
    public function simple_search($model, $searchFields)
    {
        ...
        $statusFilter = $this->get_selected_status();
        ...
        if ($statusFilter != 'default') {
            if ($statusFilter == 'saved') {
                $models->where('status', 'SAVED');
            }
            else{
                $models->where('status', 'POSTED');
            }
        }
...
//                dd($searchFields);
    ...
    public function get_filter($searchFields)
    {
        ...
        $parameters['statusFilter'] = $this->get_selected_status();
    ...
    {
        $statusFilter = request('statusFilter') ?? $this->statusFilter;
        return $statusFilter == 'default' ? 'default' : ($statusFilter == 'saved' ? 'saved' : 'posted');
    }
