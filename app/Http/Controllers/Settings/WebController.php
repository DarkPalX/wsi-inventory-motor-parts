...
class WebController extends Controller
{
    ...
    public function update_modal_content(Request $request)
    {
        $privacy = Setting::first();
        $privacy->modal_title = $request->modal_title;
        $privacy->modal_content = $request->content2;
        $privacy->modal_status = $request->has('visibility') ? 'PUBLISHED' : 'PRIVATE';
        $privacy->user_id = Auth::id();
        $privacy->save();

        return back()->with('success', __('standard.settings.website.modal_content_updates_success'));
    }
