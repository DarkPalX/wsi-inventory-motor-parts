                    ...
                    <tr>
                        ...
                        <td>
                         ...
                         @if(RolePermission::has_permission(2,auth()->user()->role_id,1))
                            <td align="right"><a href="{{ route('receiving.transactions.create') }}" class="btn text-white" style="font-size:14px; background-color: #0d6efd;">Create New Receiving</a></td>
                        @endif
                    </tr>
                    <tr><td><a href="{{route('receiving.transactions.index')}}" style="font-size:12px;">Reset Filter</a></td></tr>
                </table>
            </form>
                    ...
                    <tbody style="font-size:12px !important;">
                        @forelse ($transactions as $transaction)
                            <tr id="row{{$transaction->id}}">                                
                                <td valign="middle"><strong><a href="{{ route('receiving.transactions.show', ['id' => $transaction->id]) }}">{{ $transaction->ref_no }}</a></strong></td>
                                <td valign="middle">{{ $transaction->po_number }}</td>
                                <td valign="middle">{{ $transaction->si_number }}</td>
                                <td valign="middle">{{ (new DateTime($transaction->date_received))->format('M d, Y') }}</td>
                                <td valign="middle" width="20%"><small>{!! \App\Models\Custom\ReceivingHeader::suppliers_name($transaction->id) !!}</td>
                                ...
                                <td valign="middle">
                                    <a href="{{ route('receiving.transactions.show', ['id' => $transaction->id]) }}" class="btn btn-light text-primary" title="View Transaction"><i class="bi-eye"></i></a>
                                            ...
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    @if($transaction->status == 'SAVED' || $transaction->status == 'POSTED')
                                                        @if(RolePermission::has_permission(2,auth()->user()->role_id,1))
                                                            <li>
                                                                <a href="{{ route('receiving.transactions.edit', $transaction->id) }}" class="dropdown-item" title="Edit">
                                                                    <i class="uil-edit-alt"></i> Edit Details
                                                                </a>
                                                            </li>
                                                        @endif
                                                        @if(RolePermission::has_permission(2,auth()->user()->role_id,3) && ($transaction->status != 'CANCELLED' && $transaction->status != 'POSTED'))
                                                        <li>
                                                        ...
                                                        @endif
                                                        @if(RolePermission::has_permission(2,auth()->user()->role_id,2) && ($transaction->status != 'CANCELLED' && $transaction->status != 'POSTED'))
                                                        <li>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('receiving.transactions.index') }}";
    ...
    <script>
        ...
        function single_restore(id){
            post_form("{{ route('receiving.transactions.single-restore') }}",'',id);
        }
        ...
        function multiple_restore() {
            ...
            if (counter < 1) {
                ...
                $('.btn-restore-multiple').on('click', function() {
                    post_form("{{ route('receiving.transactions.multiple-restore') }}", '', selected_items);
                });
            }
        }
        ...
        function single_post(id){
            ...
            $('.btn-post').on('click', function() {
                post_form("{{ route('receiving.transactions.single-post') }}",'',id);
            });
        }
        ...
        function single_cancel(id){
            ...
            $('.btn-delete').on('click', function() {
                post_form("{{ route('receiving.transactions.single-delete') }}",'',id);
            });
        }
        ...
        function multiple_cancel() {
            ...
            if (counter < 1) {
                ...
                $('.btn-delete-multiple').on('click', function() {
                    post_form("{{ route('receiving.transactions.multiple-delete') }}", '', selected_items);
                });
            }
        }
