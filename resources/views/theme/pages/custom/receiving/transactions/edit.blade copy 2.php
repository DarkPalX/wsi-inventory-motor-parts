										...
										<table class="table table-hover" id="purchased_items_table">
											<thead>
												<tr>
													...
													<th width="10%"><input type="checkbox" id="select_all_vat"> VAT({{ env('VAT_RATE') }}%)</th>
											...
											<tbody>
												@foreach($receiving_details as $receiving_detail)
													<tr>
														...
														<td><input name="price[]" type="number" value="{{ $receiving_detail->price }}" step="1" min="1" onclick="this.select()" oninput="this.value = this.value < 1 ? 0 : this.value" @if($transaction->status == "POSTED") readonly @endif></td>
														<td>
															<div class="vat-container" style="display: inline-flex; align-items: center; gap: 4px;">
																<input type="checkbox" class="vat-check" {{ $receiving_detail->vat > 0 ? 'checked' : '' }}>
																...
																<input class="new-price-input border-0 text-end"
																	...
																	readonly
																	style="width:80px; {{ $receiving_detail->vat > 0 ? '' : 'display:none;' }}">
															</div>
														</td>
...
{{-- @section('pagejs')
	...
	<script>
		...
		// VAT checkbox logic
		$(document).on('change', '.vat-check', function () {
			let vatInput = $(this).closest('td').find('.vat-input');
