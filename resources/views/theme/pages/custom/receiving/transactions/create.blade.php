                        ...
                        <div class="card-body">
							...
							<form method="post" action="{{ route('receiving.transactions.store') }}" enctype="multipart/form-data" onsubmit="return checkSelectedItems();">
								...
								{{-- <div class="form-group row">
									<label class="col-sm-2 col-form-label">Suppliers</label>
									<div class="col-sm-10">
										<select class="supplier_id select-tags form-select" multiple aria-hidden="true" style="width:100%;" disabled>
											@foreach($suppliers as $supplier)
												<option value="{{ $supplier->id }}" {{ old('supplier_id') == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
											@endforeach
										</select>
										<select name="supplier_id[]" class="supplier_id form-select" multiple aria-hidden="true" style="width:100%; display:none;" required>
											@foreach($suppliers as $supplier)
												<option value="{{ $supplier->id }}" {{ old('supplier_id') == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
											@endforeach
										</select>
										...
										<table class="table table-hover" id="purchased_items_table">
											<thead>
												<tr>
													...
													<th width="10%" class="vat-col" style="display:none;"><input type="checkbox" id="select_all_vat" hidden> VAT({{ env('VAT_RATE') }}%)</th>
