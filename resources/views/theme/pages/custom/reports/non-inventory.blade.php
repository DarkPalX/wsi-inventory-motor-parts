            ...
            <div>
                <form class="d-flex align-items-center" id="searchForm" style="margin-bottom:10px;font-size: 12px !important;">
                    ...
                    <table width="100%" style="margin-bottom: 0px;">
                        ...
                        <tr><td><a href="{{route('reports.inventory')}}" style="font-size:12px;">Reset Filter</a></td></tr>
                    </table>
                </form>
            </div>
                    ...
                    <tbody>
                        @forelse ($rs as $r)
                            ...
                            <tr id="row{{$r->bid}}">
                                ...
                                <td>{{ number_format(($r->price - ($r->price * env('VAT_RATE')/100)), 2) }}</td>
                                <td>{{ number_format(($r->price * env('VAT_RATE')/100), 2) }}</td>
                                <td>{{ number_format(($r->price), 2) }}</td>
                                <td>{{ $r->created_at }}</td>
                                <td>{{ $item->Inventory }}</td>
                                <td>{{ $item->ReceivingQty }}</td>
                                <td>{{ $item->IssuedSale + $item->IssuedFree }}</td>
